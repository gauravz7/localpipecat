...
class GoogleTTSService(TTSService):
    ...
    async def run_tts(self, text: str) -> AsyncGenerator[Frame, None]:
        ...
        try:
            ...
            # Break text into chunks of approximately 100 characters
            # Try to break at sentence or punctuation boundaries when possible
            text_chunks = []
            chunk_size = 100
            
            # Identify good breaking points (end of sentence, comma, etc.)
            break_chars = ['.', '!', '?', ',', ';', ':', ' ']
            
            start = 0
            while start < len(text):
                # If remaining text is shorter than chunk_size, use it all
                if start + chunk_size >= len(text):
                    text_chunks.append(text[start:])
                    break
                    
                # Try to find a good breaking point near the chunk_size
                end = start + chunk_size
                best_break = end
                
                # Look backwards from chunk_size to find a good breaking point
                while end > start and text[end] not in break_chars:
                    end -= 1
                
                # If we found a good breaking point, use it
                if end > start:
                    best_break = end + 1 if text[end] in ['.', '!', '?'] else end
                
                text_chunks.append(text[start:best_break])
                start = best_break
            ...
            for chunk in text_chunks:
                ...
                for i in range(0, len(audio_content), CHUNK_SIZE):
                    chunk_audio = audio_content[i : i + CHUNK_SIZE]
                    if not chunk_audio:
                        break
                    await self.stop_ttfb_metrics()
                    frame = TTSAudioRawFrame(chunk_audio, self.sample_rate, 1)
                    yield frame
